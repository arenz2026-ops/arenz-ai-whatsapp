...
def receive_webhook():
    ...
    try:
        ...
        for message in messages:
            ...
            persisted_search_id = persist_conversation_turn(sender, message.get("id"), text, reply, effective_observation, criteria, stage, summary, previous, timings, turn_state)
            if inventory_store and inventory_matches is not None and persisted_search_id:
                inventory_store.record_query(persisted_search_id, message.get("id"), criteria, inventory_cutoff, inventory_matches)
            started_at = time.monotonic()
